...
def find_index_of_minimum(lst, start_index=0):
    ...
    for numbers in range(start_index, len(lst)):
        if lst[numbers] < minimum:
            minimum = lst[numbers]
            index_of_minimum = numbers
...
def selection_sort(lst):
    ...
    for numbers in range(len(lst_sorted)):
        nextSmallest = find_index_of_minimum(lst_sorted, numbers)
        swap(lst_sorted, numbers, nextSmallest)
